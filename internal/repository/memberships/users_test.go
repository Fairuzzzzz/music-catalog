...
func Test_repository_GetUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) {
				mock.ExpectQuery(`SELECT \* FROM "users" .+`).WithArgs(args.email, args.username, args.id).WillReturnRows(
		...
		},
		{
			...
			wantErr: true,
			mockFn: func(args args) {
				mock.ExpectQuery(`SELECT \* FROM "users" .+`).WithArgs(args.email, args.username, args.id).WillReturnError(assert.AnError)
			},
		},
	}
	for _, tt := range tests {
